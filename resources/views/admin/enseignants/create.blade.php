            ...
            <div class="p-6 bg-white border-b border-gray-200">
                <form method="POST" action="{{ route('admin.enseignants.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="cours" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" 
                                name="cours[]" multiple>
                            @foreach($cours as $c)
                                <option value="{{ $c->id }}" {{ (is_array(old('cours')) && in_array($c->id, old('cours'))) ? 'selected' : '' }}>
                                    {{ $c->intitule }}
                                </option>
                            @endforeach
                        </select>
